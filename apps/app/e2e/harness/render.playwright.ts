...
/** Optional one-type filter for showing flat-image smoke cases one at a time. */
const TYPE_FILTER = process.env.HARNESS_TYPE;
...
test("render and score", async ({ page }) => {
  ...
  const records = TYPE_FILTER
    ? input.records.filter((rec) => rec.expectedType === TYPE_FILTER)
    : input.records;
  if (TYPE_FILTER && records.length === 0) {
    throw new Error(`HARNESS_TYPE=${TYPE_FILTER} matched no records in ${IN}`);
  }
  ...
  L.push(
    `Model \`${input.meta.model}\`, arms ${input.meta.arms.map((a) => `\`${a}\``).join(", ")}, ` +
      `${records.length} records${TYPE_FILTER ? ` (filtered to \`${TYPE_FILTER}\`)` : ""}.`,
    "",
