...
If the source asks for grouping, use a small semantic `classDef` palette or a
separate state fence rather than inventing boundaries. Include `[ * ]` start and
end markers, and transition triggers, when the source describes lifecycle
boundaries or triggers; do not invent either for a cyclic or triggerless model.
