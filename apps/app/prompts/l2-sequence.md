...
labels short and on one line. A label may contain a path or status, but avoid
using a second colon or an unclosed quote in it.
...
- A phase or cross-participant explanation can use `Note over A,B: text`.
  Keep notes concise and attach them only to declared participants.
