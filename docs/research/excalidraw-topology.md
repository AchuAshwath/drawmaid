...
## 7. Findings table: post-processable, banned, or accepted

Every finding from §§1–6, classified under the map's escape-hatch policy: the library is not forked,
so each finding is either **(a) post-processable** on the returned skeleton or elements,
**(b) banned** from the prompt vocabulary, or **(c) accepted** as-is. Where (a) applies, the
concrete step is given.

Post-processing has two distinct insertion points, and the difference matters:

- **Pre-flight** — a validator on the mermaid _text_, before `parseMermaidToExcalidraw`. Required
  for anything that throws inside the library, because there is no skeleton to repair.
- **Post-skeleton** — on the `ExcalidrawElementSkeleton[]` returned by `parseMermaidToExcalidraw`,
  or on the `ExcalidrawElement[]` returned by `convertToExcalidrawElements`. Both are plain data.

Our call site (`apps/app/lib/canvas/insert-mermaid-into-canvas.ts:143-147`) currently does neither
and has no `try`/`catch`.

| #   | Finding                                                                                                         | §       | Class                                          | Action                                                                                                                                                                                                                                       |
| --- | --------------------------------------------------------------------------------------------------------------- | ------- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `flowchart: { curve: "linear" }` is load-bearing; the flowchart edge reader splits on `"LM"` only               | 1, 5A.1 | **(c) accept**                                 | Never pass a `MermaidConfig` with `flowchart.curve`. If a config is ever threaded through, assert `curve === "linear"` at the call site.                                                                                                     |
| 2   | Unscrapeable vertex silently returns `undefined` and is skipped                                                 | 2B      | **(a) post-skeleton**                          | Parse the mermaid source for declared node ids; compare the count against non-arrow, non-subgraph skeleton elements. Mismatch → regenerate. Geometry cannot be reconstructed, so detection is the whole fix.                                 |
| 3   | Edge dropped when its path yields ≤ 1 reflection point                                                          | 2B      | **(a) post-skeleton**                          | Same detector on edge count vs `type === "arrow"` element count.                                                                                                                                                                             |
| 4   | One unscrapeable node cascades: every edge touching it is discarded                                             | 2C      | **(a) post-skeleton**                          | Covered by rows 2 and 3 together; the cascade is why counting both is necessary rather than either alone.                                                                                                                                    |
| 5   | `graphImage` fallback returns a single flat `image` element                                                     | 2C      | **(a) post-skeleton**                          | At the call site, before `convertToExcalidrawElements`: reject when `skeleton.length === 1 && skeleton[0].type === "image"`. This is the single highest-value guard in the table.                                                            |
| 6   | Binding failure is a no-op: arrow emitted with `startBinding: null` and only a `console.error`                  | 3.2     | **(a) post-conversion**                        | After `convertToExcalidrawElements`, for each arrow with a null binding, resolve the intended shape from the source edge and write `startBinding`/`endBinding` **and** the shape's `boundElements` entry. All three are plain fields (§3.3). |
| 7   | Bindings survive dragging unconditionally                                                                       | 3.4     | **(c) accept**                                 | A capability, not a defect. It deletes the whole family of geometric prompt rules (§6 preamble). Record it so nobody re-derives it.                                                                                                          |
| 8   | Every mermaid node shape converts to a bindable element type                                                    | 3.3     | **(c) accept**                                 | Node shape is never the reason a binding fails. Shape selection is #34's problem, not topology's.                                                                                                                                            |
| 9   | Disconnected component floats into an unrelated component's rank                                                | 4.2     | **(a) post-conversion** + soft ban             | Compute components from the source and translate each component's elements as a block — the call site already does bulk translation at `insert-mermaid-into-canvas.ts:109-127`. Also ask the prompt for one component (I6).                  |
| 10  | Origin collapse: `translate()` regex cannot read scientific notation                                            | 4.3     | **(a) post-skeleton**                          | Reject any element at exactly `x === 0 && y === 0`, or whose bbox is disjoint from the union of the others'. Regeneration trigger. No mermaid construct causes it, so nothing is banned for it.                                              |
| 11  | Ancestor check `root.classList.value === "root"` is exact equality                                              | 4.3     | **(c) accept**                                 | Holds today because mermaid writes that class alone. Row 10's detector catches it if mermaid ever adds a second class. Nothing to do now.                                                                                                    |
| 12  | Self-loop `A --> A` produces an arrow bound to one shape at both ends                                           | 4.4     | **(b) ban** (I2)                               | Reject in the pre-flight validator. Revisit only if #46 shows the drag behaviour is acceptable.                                                                                                                                              |
| 13  | `-` in a node id → `domId` substring ambiguity → two nodes on one bbox                                          | 5.1     | **(b) ban** (I1)                               | Pre-flight. This is the only mechanism found that genuinely produces overlapping nodes.                                                                                                                                                      |
| 14  | `"` or `\` in a node id → `querySelector` `SyntaxError` → image fallback                                        | 5.1     | **(b) ban** (I1)                               | Pre-flight. Not post-processable: the throw is inside the library and the caller only sees an image.                                                                                                                                         |
| 15  | `_` in a node id → arrow-id collision → `TypeError` out of `convertToExcalidrawElements`                        | 5.2     | **(b) ban** (I1)                               | Pre-flight. Explicitly contradicts #31 Task 2's proposed `api_gw`-style ids; that task needs rewriting from this.                                                                                                                            |
| 16  | `regenerateIds: false` would silently drop the duplicate instead of crashing                                    | 5.2     | **(c) accept**                                 | Keep `regenerateIds: true`. A crash we can catch beats a silent drop we cannot see.                                                                                                                                                          |
| 17  | Parallel edges share one arrow id `A_B`                                                                         | 5.3     | **(c) accept**, provisionally                  | Benign today because nothing binds _to_ an arrow. Flips to **(b) ban a second edge per ordered pair** if #46 shows either arrow losing a binding.                                                                                            |
| 18  | `encodeEntities` divergence: db parsed from encoded text, SVG from raw                                          | 5.4     | **(b) ban** `#` and `;` in ids and labels (I1) | Pre-flight. The divergence surfaces as a `querySelector` miss, i.e. row 2's silent drop, so post-processing sees only a missing node.                                                                                                        |
| 19  | Reserved words (`graph`, `end`, `class`, `click`, …) and edge-head chars `o` / `x`                              | 5.5     | **(b) ban** (I1)                               | Pre-flight. These fail upstream of everything else, at parse time.                                                                                                                                                                           |
| 20  | 2.2.2: a mermaid render/draw failure resolves to an `image` of the error graphic rather than rejecting          | 5A.2    | **(a) post-skeleton**                          | Row 5's check, promoted from defensive to mandatory. Without it, a syntax error is inserted onto the canvas as a picture of a syntax error.                                                                                                  |
| 21  | 2.2.2: `getDiagramFromText` still throws for pure parse errors                                                  | 5A.2    | **(c) accept**                                 | Wrap the call site in `try`/`catch` so the two failure shapes (reject, and row 20's resolve-with-image) are handled by one recovery path.                                                                                                    |
| 22  | Subgraph miss throws `SubGraph element not found` → whole diagram becomes one image                             | 5A.3    | **(a) post-skeleton** + tier gate              | Detected by row 5. Because the failure is total rather than partial, gate `subgraph` to Rich/Deep (§6.3) instead of putting it in the shared vocabulary.                                                                                     |
| 23  | Subgraph selector is single-quoted, so `'` in a subgraph id is a `SyntaxError`                                  | 5A.3    | **(b) ban** (I3)                               | Pre-flight, on subgraph ids specifically. Vertices are broken by `"`; subgraphs by `'`.                                                                                                                                                      |
| 24  | 2.2.2: post-conversion dedupe at 0.5px can reduce a 2-point arrow to 1 point                                    | 5A.4    | **(a) post-conversion**                        | Drop or repair any arrow with `points.length < 2` before insertion. The parser's `reflectionPoints > 1` filter runs earlier and cannot catch this.                                                                                           |
| 25  | 2.2.2: subgraph rectangles widened from an estimated label width and re-centred, so they can overlap neighbours | 5A.5    | **(a) post-conversion** + prompt cap           | The widening is deterministic — `max(20, ceil(len * fontSize * 0.62)) + 64` — so it can be recomputed and undone, or neighbours nudged. Also cap subgraph title length in the prompt; that is free.                                          |
| 26  | Subgraph id and node id share one namespace in the converter                                                    | 6.2 I3  | **(b) ban** collisions                         | Pre-flight: no subgraph id may equal a node id.                                                                                                                                                                                              |
| 27  | An edge naming a subgraph id binds to the cluster rectangle while its geometry comes from an inner node         | 6.2 I4  | **(b) ban**                                    | Pre-flight. Post-processing could in principle re-point the binding, but the arrow's _points_ still describe a path to a different element, so the repair is cosmetic at best.                                                               |
| 28  | No geometric condition governs binding permanence                                                               | 6       | **(c) accept**                                 | Do not write spacing, gap, or edge-length rules into any prompt layer.                                                                                                                                                                       |
| 29  | Cycles are handled by dagre's `acyclic.run` / `undo`                                                            | 6.1     | **(c) accept**                                 | No acyclicity rule in the prompt.                                                                                                                                                                                                            |
| 30  | `maxEdges: 500`, `maxTextSize: 50000`                                                                           | 6.1     | **(c) accept**                                 | Unreachable within any tier's token budget. Exceeding either rejects rather than degrading, so no guard is needed.                                                                                                                           |
| 31  | Every failure in the pipeline is silent or near-silent                                                          | 6.2 I5  | **(a) post-skeleton**                          | Install a `console.error` spy around the conversion and treat any captured error as a regeneration trigger. §3.2's `No element for start binding with id … found` is the only in-band signal that exists.                                    |

### 7.1 What this collapses to

Three artefacts, and nothing else:

1. **A pre-flight mermaid validator** (rows 12–15, 18, 19, 23, 26, 27). One regex for ids, one
   reserved-word set, one self-loop check, one subgraph-id check, one edge-endpoint check. Runs on
   the model's output text before the converter sees it. This is where every unfixable failure is
   stopped, because it is the last point at which we still have something to reject.
2. **A skeleton acceptance check** (rows 2–5, 10, 20, 22, 31). Counts, the single-`image` guard, the
   origin guard, and a console-error spy. Its output is a boolean: insert, or regenerate.
3. **An element repair pass** (rows 6, 9, 24, 25). Re-bind, re-lay-out components, drop degenerate
